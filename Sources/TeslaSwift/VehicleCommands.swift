...
public enum VehicleCommand {
    ...
    case remoteBoombox
    case setClimateKeeperMode(mode: ClimateKeeperMode)
    case setSeatCooler(position: SeatPosition, level: CoolerLevel)
    case setSeatHeaterAuto(seat: AutoHeatedSeat, on: Bool)
    case setBioweaponMode(on: Bool)
    ...
    func path() -> String {
        ...
        case .setSeatHeaterAuto:
            return "command/remote_auto_seat_climate_request"
        case .setSeatCooler:
            return "command/remote_seat_cooler_request"
        case .setBioweaponMode:
            return "command/set_bioweapon_mode"
        }
    }
}
